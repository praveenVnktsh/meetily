        ...
        <div className="flex flex-col items-center space-y-4 mt-4">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                onClick={() => router.push('/')}
                className={`p-2 rounded-lg transition-colors duration-150 ${isHomePage ? 'bg-surface-2' : 'hover:bg-surface-2'
                  }`}
              >
                <Home className="w-5 h-5 text-ink-muted" />
              </button>
            </TooltipTrigger>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                ...
                className={`p-2 rounded-lg transition-colors duration-150 ${isMeetingPage ? 'bg-surface-2' : 'hover:bg-surface-2'
                  }`}
              >
                <NotebookPen className="w-5 h-5 text-ink-muted" />
              </button>
            </TooltipTrigger>
            <TooltipContent side="right">
              <p>Meeting Notes</p>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                ...
                className={`p-2 rounded-lg transition-colors duration-150 ${isSettingsPage ? 'bg-surface-2' : 'hover:bg-surface-2'
                  }`}
              >
        ...
        <div
          className={`flex items-center transition-all duration-150 group ${item.type === 'folder' && depth === 0
            ...
            : `px-3 py-2 my-0.5 rounded-md text-sm ${isActive ? 'bg-blue-100 text-blue-700 font-medium' :
              hasTranscriptMatch ? 'bg-yellow-50' : 'hover:bg-surface-2'
            } cursor-pointer`
            }`}
          ...
          {item.type === 'folder' ? (
            ...
            <div className="flex flex-col w-full">
              <div className="flex items-center w-full">
                {isMeetingItem ? (
                  <div className="flex-shrink-0 flex items-center justify-center w-6 h-6 rounded-full mr-2 bg-surface-2">
                    <File className="w-3.5 h-3.5 text-ink-muted" />
                  </div>
                ) : (
              ...
              {hasTranscriptMatch && (
                <div className="mt-1 ml-8 text-xs text-ink-muted bg-yellow-50 p-1.5 rounded border border-yellow-100 line-clamp-2">
  ...
  return (
    <div className="fixed top-0 left-0 h-screen z-40">
      ...
      <button
        ...
        className="absolute -right-6 top-20 z-50 p-1 bg-surface-raised hover:bg-surface-2 rounded-full shadow-lg border"
      ...
      <div
        className={`h-screen bg-surface-raised border-r shadow-sm flex flex-col transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'
          }`}
      >
        ...
        <div className="flex-shrink-0 h-22 flex items-center">
          ...
          <div className="flex-1">
            {!isCollapsed && (
              <div className="p-3">
                {/* <span className="text-lg text-center border rounded-full bg-blue-50 border-white font-semibold text-ink mb-2 block items-center">
                  <span>Minutes</span>
                </span> */}
                <Logo isCollapsed={isCollapsed} />
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <div className="flex-shrink-0">
            {!isCollapsed && (
              <div
                onClick={() => router.push('/')}
                className="p-3  text-lg font-semibold items-center hover:bg-surface-2 h-10   flex mx-3 mt-3 rounded-lg cursor-pointer"
              >
              ...
              <div className="flex-shrink-0">
                {filteredSidebarItems.filter(item => item.type === 'folder').map(item => (
                  <div key={item.id}>
                    ...
                    >
                      <NotebookPen className="w-4 h-4 mr-2 text-ink-muted" />
        ...
        {!isCollapsed && (
          ...
          <div className="flex-shrink-0 p-2 border-t border-hairline">
            ...
            {betaFeatures.importAndRetranscribe && (
              <button
                onClick={() => openImportDialog()}
                className="w-full flex items-center justify-center px-3 py-2 mt-1 text-sm font-medium text-ink bg-blue-100 hover:bg-blue-200 rounded-lg transition-colors shadow-sm"
              >
            ...
            <button
              onClick={() => router.push('/settings')}
              className="w-full flex items-center justify-center px-3 py-1.5 mt-1 mb-1 text-sm font-medium text-ink bg-surface-2 hover:bg-surface-2 rounded-lg transition-colors shadow-sm"
            >
              <Settings className="w-4 h-4 mr-2" />
              <span>Settings</span>
            </button>
            <Info isCollapsed={isCollapsed} />
            <div className="w-full flex items-center justify-center px-3 py-1 text-xs text-ink-subtle">
              v0.6.0
            </div>
          </div>
        )}
        ...
        <DialogContent className="sm:max-w-[425px]">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-hairline rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          ...
          <DialogFooter>
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-ink bg-surface-2 hover:bg-surface-2 rounded-md transition-colors"
            >
              Cancel
            </button>
            <button
