  ...
  return (
    <div 
      className={`group relative min-h-[24px] flex items-start rounded transition-all duration-150 ease-in-out
        ${isSelected ? 'bg-blue-50 ring-1 ring-blue-200 shadow-sm' : 'hover:bg-surface-2'}`}
    ...
    >
      ...
      <div className="relative flex-1 py-0.5 px-1">
        ...
        {showCommands && (
          <div 
            ref={commandsRef}
            className="absolute left-0 top-full mt-1 w-64 bg-surface-raised rounded-lg shadow-lg border border-hairline py-2 z-50
          ...
          >
            {filteredCommands.map((cmd, index) => (
              <button
                key={cmd.id}
                className={`
                  w-full text-left px-3 py-2 flex items-center space-x-3 hover:bg-surface-2
                  ${index === selectedCommandIndex ? 'bg-surface-2' : ''}
                `}
                onClick={() => handleCommandSelect(cmd)}
                onMouseEnter={() => setSelectedCommandIndex(index)}
              >
                <span className="flex-shrink-0 w-6 h-6 flex items-center justify-center bg-surface-2 rounded text-ink-muted">
                  {cmd.icon}
                </span>
