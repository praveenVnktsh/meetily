  ...
  return (
    ...
    >
      <div className="flex flex-col items-center space-y-10">
        {/* Divider */}
        <div className="w-16 h-px bg-surface-2" />

        {/* Features Card */}
        <div className="w-full max-w-md bg-surface-raised rounded-lg border border-hairline shadow-sm p-6 space-y-4">
          {features.map((feature, index) => {
            ...
            return (
              <div key={index} className="flex items-start gap-3">
                ...
                <p className="text-sm text-ink leading-relaxed">{feature.title}</p>
              </div>
            );
          })}
        </div>
