...
export function ChunkProgressDisplay({
  ...
  const getChunkStatusColor = (status: ChunkStatus['status']) => {
    switch (status) {
      ...
      case 'pending':
      default:
        return 'text-ink-muted bg-surface-2 border-hairline';
    }
  };

  return (
    <div className={`bg-surface-raised border border-hairline rounded-lg p-4 ${className}`}>
      {/* Progress Header */}
...
export function ChunkProgressMini({ progress, className = '' }: { progress: ProcessingProgress; className?: string }) {
  ...
  return (
    <div className={`bg-surface-2 border border-hairline rounded-lg p-3 ${className}`}>
      <div className="flex items-center justify-between mb-2">
        <span className="text-sm font-medium text-ink">
          Processing
        </span>
